...
module.exports = class extends think.Controller {
  ...
  async getAction() {
    ...
    if (this.id) {
      ...
      data = await this.modelInstance.where({ [pk]: this.id, status: 1 }).find();
      return this.success(data);
    }
    if (this.modelInstance.modelName === 'images') {
      data = await this.modelInstance.where({ status: 1 }).select();
    } else {
      data = await this.modelInstance.where({ status: 1 }).order({sort: 'DESC', id: 'DESC'}).select();
    }
    return this.success(data);
  }
  /**
